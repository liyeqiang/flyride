...
Page({
  ...
  onLoad(options) {
    ...
    // 从 URL Scheme 回跳时带 insuranceDone=1
    if (insuranceDone === '1') {
  ...
  async _loadOrder() {
    try {
      ...
      if (order.error) { wx.showToast({ title: order.error, icon: 'none' }); return; }
  ...
  _startExpireCountdown(expireAt) {
    ...
    const tick = () => {
      ...
      if (ms <= 0) { this.setData({ expireText: '已超时' }); clearInterval(this._expireTimer); return; }
